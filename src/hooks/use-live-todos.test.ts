...
describe("useLiveTodos", () => {
  ...
  it("should initialize with loading state and subscribe to todos", async () => {
    ...
    (
      todoService.subscribeToToDos as ReturnType<typeof vi.fn>
  ...
  it("should update filter constraints when search params change", async () => {
    ...
    expect(todoService.subscribeToToDos).toHaveBeenCalledWith(
      expect.any(Function),
      expect.any(Function),
      ["mockConstraint"],
    );
  ...
  it("should unsubscribe when unmounted", () => {
    ...
    (todoService.subscribeToToDos as ReturnType<typeof vi.fn>).mockReturnValue(
      unsubscribeMock,
    );
